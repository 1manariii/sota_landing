    ...
    return (
        <div className={`${styles.featureColumn} ${isLoaded ? styles.loaded : ''}`} style={{ animationDelay: `${index * 150}ms` }}>
            <div className={styles.featureHeader}>
                {/* <span className={styles.featureIcon}>{column.icon}</span> */}
                <img className={styles.featureImg} src={column.img} />
            ...
            <div className={`${styles.featuresContent} container`}>
                ...
                <div className={styles.featuresGrid}>
                    {FEATURE_COLUMNS.map((column, index) => (
                        <FeatureColumnCard key={column.title} column={column} index={index} isLoaded={isLoaded} />
                    ))}
                </div>
