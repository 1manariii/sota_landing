...
// --- Типы данных (согласно твоему API) ---
...
interface Product {
  ...
  name: string;
  description: string;
  // Картинки в этом эндпоинте нет, но можно добавить заглушку или брать из категории, если нужно
  image?: string; 
}
...
// Компонент для обработки кликов по карте (закрытие панели)
...
export const MapProducts = () => {
  ...
  // 1. Загрузка списка платформ при монтировании
  useEffect(() => {
  ...
  // 2. Загрузка товаров при выборе платформы
  useEffect(() => {
  ...
  // Находим данные выбранной платформы для отображения заголовка
  ...
  return (
    <section className={styles.mapSection}>
      <div className={`${styles.mapContainer} ${selectedPlatformId ? styles.shifted : ''}`}>
        <MapContainer 
          center={[55.7558, 37.6173]} // Центр России (можно улучшить до геолокации пользователя)
          zoom={5} 
              ...
              {loadingProducts ? (
                ...
                products.map((product) => (
                  <div key={product.id} className={styles.productCard}>
                    {/* Так как в API продуктов нет картинки, ставим заглушку или иконку */}
